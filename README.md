...
**Content filtering on a public demo.** Every free-text write path (manual entry, CSV import, AI extraction) runs through a local profanity filter (`obscenity`) before it's persisted — user-entered text is rejected with a validation error, AI-extracted text is redacted to keep the lenient partial-extraction UX. An optional Discord webhook (`DISCORD_WEBHOOK_URL`) pings on new bill/vendor creation so anything that slips through gets caught fast. Not a claim of complete moderation coverage — a deterrent appropriate for a portfolio demo, not a production trust & safety system.
